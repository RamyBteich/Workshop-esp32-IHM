...
### Explication complète de l’affichage, du tactile, du buffering et de l’intégration SquareLine


##  Vue d’ensemble

Ce document explique en détail comment un **M5Core2** (basé sur ESP32) utilise la bibliothèque graphique **LVGL** pour :

- Initialiser l’écran et l’entrée tactile  
- Configurer les buffers de dessin LVGL  
- Gérer le temps interne de LVGL (ticks)  
- Charger une interface graphique générée par **SquareLine Studio** (`ui.h`, `ui_init()`)  
- Mettre à jour l’interface en continu dans la boucle `loop()` d’Arduino  
